...
//SEND UNCONFIRMED PM
	...
	if ($vbulletin->options['dbtech_vbdonate_waitpm_enable'] AND $vbulletin->userinfo['userid'] AND $vbulletin->options['dbtech_vbdonate_pm_enable'])
	{
		$pmsender = fetch_userinfo($vbulletin->options['dbtech_vbdonate_pm_sender']);
		$pmreceiver = fetch_userinfo($vbulletin->userinfo['userid']);
		$subject = $vbphrase['dbtech_vbdonate_pmreport_wait_title'];
		$message = construct_phrase($vbphrase['dbtech_vbdonate_pmreport_wait_message'],$pmreceiver['username'],vbdate($dbt_vbd_addauto_dtformat, TIMENOW));
