...
//SEND PM TO DONATOR MANUALLY ADDED DONATIONS
	if ($vbulletin->options['dbtech_vbdonate_thankspm_man_enable'] AND $vbulletin->GPC['userid'] AND $vbulletin->options['dbtech_vbdonate_pm_enable'])
	{
		...
		{
			$subject = $vbphrase['dbtech_vbdonate_pmreport_thanks_manual_title'];
			$message = construct_phrase($vbphrase['dbtech_vbdonate_pmreport_thanks_manual_message'],$pmreceiver['username'],vbdate($dbt_vbd_addman_dtformat,$dbt_vbd_new_dateline),$vbulletin->options['dbtech_vbdonate_currency'],$vbulletin->GPC['amount']);
