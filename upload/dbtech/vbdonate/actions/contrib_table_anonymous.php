...
\*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
	...
	if (($_REQUEST['do'] == 'contrib_table_anonymous') AND !$vbulletin->options['dbtech_vbdonate_enable'])
	{
		eval(standard_error($vbulletin->options['dbtech_vbdonate_closedreason']));
	}
	if (($_REQUEST['do'] == 'contrib_table_anonymous') AND $vbulletin->options['dbtech_vbdonate_disable'])
	{
		eval(standard_error($vbulletin->options['dbtech_vbdonate_closedreason']));			
	}                                                            
	...
	$dbt_vbd_ppl_nrinfo = $vbulletin->db->query_read(" SELECT id, amount, netamount, confirmed FROM " . TABLE_PREFIX . "dbtech_vbdonate_donations AS dbtech_vbdonate_donations WHERE dbtech_vbdonate_donations.anonymous = 1");
	...
	$dbt_vbd_ppl_info = $vbulletin->db->query_read("
		...
		WHERE dbtech_vbdonate_donations.anonymous = 1
	...
	while ($dbt_vbd_contrib_table = $vbulletin->db->fetch_array($dbt_vbd_ppl_info))
	{
		...
		else
		{
			...
			$templater = vB_Template::Create('dbtech_vbdonate_contrib_table_anon_bit');
	...
	$pagenav = construct_page_nav($pagenumber, $perpage, $dbt_vbd_ppl_nr, 'vbdonate.php?' . $vbulletin->session->vars['sessionurl'] . 'do=contrib_table_anonymous&amp;cs='.$vbulletin->GPC['cs'].'&amp;co='.$vbulletin->GPC['co'], "");

	$navbits = construct_navbits(array('' => $vbphrase['dbtech_vbdonate_donations']));
	$navbar = render_navbar_template($navbits);

	$templater = vB_Template::Create('dbtech_vbdonate_contrib_table_anon');
